...
class LookupElementFactory(
    ...
    private fun callableWeight(descriptor: DeclarationDescriptor): CallableWeight? {
        ...
        if (descriptor.original.overriddenDescriptors.isNotEmpty()) {
            ...
            return overridden.map { callableWeightNoOverridden(it, receiverTypes)!! }.min()!!
        }

        return callableWeightNoOverridden(descriptor, receiverTypes)
    }

    private fun callableWeightNoOverridden(descriptor: CallableDescriptor, receiverTypes: Collection<KotlinType>): CallableWeight? {
