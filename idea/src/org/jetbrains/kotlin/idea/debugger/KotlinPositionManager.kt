...
class KotlinPositionManager(private val myDebugProcess: DebugProcess) : MultiRequestPositionManager, PositionManagerEx() {
    ...
    override fun getAllClasses(sourcePosition: SourcePosition): List<ReferenceType> {
        ...
        if (psiFile is ClsFileImpl) {
            val decompiledPsiFile = psiFile.read { it.decompiledPsiFile }
    ...
    private fun classNamesForPosition(sourcePosition: SourcePosition, withInlines: Boolean): List<String> {
        val element = sourcePosition.read { it.elementAt } ?: return emptyList()
    ...
    private fun classNamesForPosition(element: PsiElement?, withInlines: Boolean): List<String> {
        if (DumbService.getInstance(myDebugProcess.project).isDumb) {
            ...
            return getOrComputeClassNames(baseElement) {
                element ->
                val file = element.read { it.containingFile as KtFile }
    ...
    private fun findLambdas(sourcePosition: SourcePosition): Collection<String> {
        val lambdas = sourcePosition.read { getLambdasAtLineIfAny(it) }
    ...
    private fun getInternalClassNameForElement(
        ...
        val parent = element.read { getElementToCalculateClassName(it.parent) }
        when (element) {
            is KtClassOrObject -> return CachedClassNames(getClassNameForClass(element, typeMapper))
            is KtFunction -> {
                val descriptor = element.read { InlineUtil.getInlineArgumentDescriptor(it, typeMapper.bindingContext) }
                if (descriptor != null) {
        ...
        val crossInlineParameterUsages = element.read { it.containsCrossInlineParameterUsages(typeMapper.bindingContext) }
        ...
        when {
            ...
            element is KtPropertyAccessor && (!element.read { it.property.isTopLevel } || !isInLibrary) -> {
                val classOrObject = element.read { PsiTreeUtil.getParentOfType(it, KtClassOrObject::class.java) }
                if (classOrObject != null) {
                    return CachedClassNames(getClassNameForClass(classOrObject, typeMapper))
                }
            }
            element is KtProperty && (!element.read { it.isTopLevel } || !isInLibrary) -> {
    ...
    private fun getClassNameForClass(klass: KtClassOrObject, typeMapper: JetTypeMapper) = klass.read { getJvmInternalNameForImpl(typeMapper, it) }
    private fun getClassNameForFile(file: KtFile) = file.read { NoResolveFileClassesProvider.getFileClassInternalName(it) }
    ...
    private fun getElementToCalculateClassName(notPositionedElement: PsiElement?): KtElement? {
        ...
        return read { PsiTreeUtil.getParentOfType(notPositionedElement, *TYPES_TO_CALCULATE_CLASSNAME) }
    }
...
private inline fun <U, V> U.read(crossinline f: (U) -> V): V {
