...
private fun KotlinType.dependentDescriptors(types: Set<KotlinType>, ownRelation: RelationToType): Set<DescriptorWithRelation> =
        this.constructor.declarationDescriptor.let {
            if (this in types) emptySet()
            else (it?.let { it.dependentDescriptors(ownRelation) } ?: emptySet()).let {
                it + arguments.map { it.type.dependentDescriptors(types + this, ARGUMENT)}.flatten()
            }
        }

fun Set<DescriptorWithRelation>.effectiveVisibility() = lowerBound(this.map { it.effectiveVisibility() })
