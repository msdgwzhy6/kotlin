...
class ErasedOverridabilityCondition : ExternalOverridabilityCondition {
    override fun isOverridable(superDescriptor: CallableDescriptor, subDescriptor: CallableDescriptor, subClassDescriptor: ClassDescriptor?): Result {
        ...
        val basicOverridability = OverridingUtil.getBasicOverridability(superDescriptor, subDescriptor)?.result
        if (basicOverridability != null
                && basicOverridability != OverridingUtil.OverrideCompatibilityInfo.Result.OVERRIDABLE) return Result.UNKNOWN
